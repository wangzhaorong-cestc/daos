...
func (mod *mgmtModule) getCachedInterface(ctx context.Context, params *fiParams) (*FabricInterface, error) {
	if params.interfaceName != "" {
		result, err := mod.fabricInfo.localNUMAFabric.Find(params.interfaceName)
		if err != nil {
			return nil, err
		}

		if params.interfaceDomain != "" && result.Domain != params.interfaceDomain {
			return nil, errors.Errorf("domain %q for fabric interface %q doesn't match requested %q",
				result.Name, result.Domain, params.interfaceDomain)
		}
