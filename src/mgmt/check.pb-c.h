...
struct  Mgmt__CheckQueryPool
{
  ...
  uint32_t phase;
  /*
   * The count of total found inconsistency on the pool (shard).
   */
  uint32_t inconsist_cnt;
  /*
   * The count of repaired inconsistency on the pool (shard).
   */
  uint32_t repaired_cnt;
  /*
   * The count of ignored inconsistency on the pool (shard).
   */
  uint32_t ignored_cnt;
  /*
   * The count of fail to repaired inconsistency on the pool (shard).
   */
  uint32_t failed_cnt;
  /*
   * The time of check instance being started on the pool (shard).
   */
  uint64_t start_time;
  /*
...
#define MGMT__CHECK_QUERY_POOL__INIT \
 { PROTOBUF_C_MESSAGE_INIT (&mgmt__check_query_pool__descriptor) \
    , (char *)protobuf_c_empty_string, (char *)protobuf_c_empty_string, 0, 0, 0, 0, 0, 0, 0, 0 }
...
struct  Mgmt__CheckQueryTarget
{
  ...
  uint32_t status;
  /*
   * Query results for pool shards on this target.
   */
  size_t n_shards;
  Mgmt__CheckQueryPool **shards;
};
